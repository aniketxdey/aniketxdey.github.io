...
An adversarial attack that reconstructs private voice samples from speech-recognition models trained under federated learning. Matches gradients through the CTC loss, recovering 99% of the original training audio federated protocols are meant to protect, validated on DeepSpeech2, a production ASR framework.
...
A suite of computational bioinformatics tools that interpret single-cell RNA-seq data to identify genomic drivers in the tumor microenvironment. Clustering analysis surfaces the gene-expression shifts behind malignant transition, and a random-forest pipeline predicts ligand-receptor interactions at those sites with 89.32% accuracy to guide therapeutic targeting.
...
A diffusion-based world model to simulates 2D billiards physics with no physics engine at inference. An action-conditioned diffusion transformer predicts each frame in VAE latent space, using diffusion forcing for long-horizon stability, trained on a 200,000-episode dataset and playable in a real time neural simulator over WebSocket.
...
A non-invasive method for early dementia diagnosis that pairs NLP with machine learning. Instead of brain scans or blood tests, it derives diagnostic signals from cognitive and language assessments, enabling accessible screening for the elderly populations where cognitive impairment is most common.
...
Novel architecture I designed in an internship that measures test-case automation across corporate software infrastructure. Authenticates against a company's cloud servers, pulls test-case records, and parses them into metrics on automation coverage and status for management.
...
Real-time calibration agorithm for the Heston stochastic-volatility model to live SPY/SPX option surfaces, priced via the characteristic function with a Carr-Madan FFT and seeded by moment matching. A four-stage optimizer reaches an R² of 0.94, recalibrates in under 20ms, and cuts compute time 65% versus FFT-only implementations.
