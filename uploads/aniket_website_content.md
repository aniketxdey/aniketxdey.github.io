...
A diffusion-based world model that simulates 2D billiards physics with no physics engine at inference. An action-conditioned diffusion transformer predicts each frame in VAE latent space, and diffusion forcing (multi-step training rollouts) holds physical consistency over long horizons. Trained on a 200,000-episode dataset, with real-time play served over WebSocket.
...
A two-stage deep learning system that improves object detection for autonomous vehicles in adverse weather. A Visibility Correction Module masks and removes rain, snow, and fog through a dimensionality-reduction network, then an Object Detection Module labels objects in the cleaned frames. It reached 89.72% average accuracy, beating industry alternatives on accuracy and detection time.
...
Real-time calibration of the Heston stochastic-volatility model to live SPY/SPX option surfaces. Pricing uses the characteristic function with a Carr-Madan FFT, seeded by a moment-matching framework for analytic initial parameters. A four-stage optimizer reaches an R² of 0.94, recalibrates in under 20ms, and cuts compute time 65% versus FFT-only implementations. SABR and Black-Scholes baselines are included.
...
An options pricing platform built for Dartmouth's Mathematical Finance I (MATH 86). It pulls live market data and prices each contract with three models side by side, Black-Scholes, a Cox-Ross-Rubinstein binomial tree, and a 10,000-path Monte Carlo, to expose model disagreement. It also computes analytical and numerical Greeks, delta-neutral hedges, and full implied-volatility surfaces.
...
Martian applies agentic AI to enterprise infrastructure cost optimization. Its agents connect to AWS, Azure, GCP, and on-premises systems through native APIs, map infrastructure and storage dependencies, and execute the changes that eliminate wasted spend. A LangGraph multi-agent backend reasoning through Claude drives the analysis, fronted by a React and TypeScript interface aimed at finance and engineering teams.
...
A machine learning pipeline (Dartmouth CS74) that classifies Amazon reviews by rating and clusters them by category with scikit-learn. Text is TF-IDF vectorized, then logistic-regression and SGD classifiers are tuned per rating cutoff, beating the course Kaggle baselines at every threshold. The clustering task instead uses 40 binary category-keyword features.
